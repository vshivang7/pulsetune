...
const Home = ({setCurrentPlaylist, currentPlaylist, preQueue, setPreQueue, postQueue, setPostQueue, currentMusic, setCurrentMusic, search, user, setUser, musics}) => {
    ...
    <div className="grid gap-3 grid-cols-1 xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xs:grid-cols-2 w-full mt-5">
  {
    search ? (
      ...
      musics.map((music) => (
        <div className="m-2" key={music._id}>
          <Card
            currentPlaylist = {currentPlaylist} setCurrentPlaylist = {setCurrentPlaylist} setPostQueue = {setPostQueue} preQueue = {preQueue} setPreQueue = {setPreQueue} postQueue = {postQueue} currentMusic = {currentMusic} setCurrentMusic={setCurrentMusic} setUser={setUser} isPlaying={isPlaying} setIsPlaying={setIsPlaying} audioRef={audioRef} image={music.image} song_name={music.song_name} artist={music.artist} url={music.url} user={user} id={music._id}/>
        </div>
      ))
    )
  }
