        ...
        <div className='grid gap-3 grid-cols-1 xl:grid-cols-6 lg:grid-cols-5 md:grid-cols-4 sm:grid-cols-3 xs:grid-cols-2 w-full mt-5'>
          {playlistMusics.map((music) => (
            <div key={music._id} className="m-2">
                <PlaylistSongs setCurrentPlaylist={setCurrentPlaylist} playlistMusics = {playlistMusics} preQueue = {preQueue} setPreQueue = {setPreQueue} postQueue = {postQueue} setPostQueue = {setPostQueue} music={music} playlistID={id} setUser={setUser} currentMusic={currentMusic} setCurrentMusic={setCurrentMusic}/>
            </div>
          ))}
        </div>
      )}
